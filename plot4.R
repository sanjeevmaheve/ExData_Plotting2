...
SCC$Level <- paste(SCC$SCC.Level.One, SCC$SCC.Level.Two, 
                  SCC$SCC.Level.Three, SCC$SCC.Level.Four, sep = " ")
subsetCombSCC <- SCC[grepl("coal", ignore.case = TRUE, SCC$Level), ]
subsetCoalSCC <- subsetCombSCC[grepl("combustion", ignore.case = TRUE, SCC$Level), ]

# Join two data frames on SCC (i.e. a common field)
coalNEI <- merge(subsetCoalSCC, NEI, by="SCC")
...
plot(stats$year,
     ...
     main = "PM2.5 emission count in U.S. (coal-combustion sources)",
