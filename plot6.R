...
# Extract dataset only for Baltimore City, Maryland and Los-Angeles
BaltimoreLosAngelesNEI <- filter(NEI, fips == "24510" | fips == "06037")
...
motorNEI <- merge(motorSCC, BaltimoreLosAngelesNEI, by="SCC")
...
stats <- group_by(motorNEI, year, fips) %>% 
...
myPlot <- qplot(year, emissionsCount, 
                 data = stats, color=fips, 
                 ylab = 'Emissions Count', xlab='Year', 
                 geom = c("point", "smooth"), 
                 main = "PM2.5 emission count for vehicles in Baltimore and Los Angeles")
suppressWarnings(print(myPlot + theme_bw(base_family = "Times")))

# Save result to a file
dev.copy(png, file="plot6.png", width=580, height=580)
dev.off()